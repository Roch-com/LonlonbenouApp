...
async function essayer(cible: string): Promise<string | undefined> {
  const client = new Client({ connectionString: cible, connectionTimeoutMillis: 5000 });
  try {
...
for (const port of ['5432', '5433', '5434']) {
  const variante = url.replace(/(@[^/:]+):\d+\//, `$1:${port}/`);
  if (variante === url) continue;
  try {
    const version = await essayer(variante);
    console.log(`\n✅ Le port ${port} répond : ${version?.split(',')[0]}`);
    console.log(`   Corriger DATABASE_URL dans apps/api/.env pour pointer sur ${port}.`);
    process.exit(0);
  } catch (erreur) {
